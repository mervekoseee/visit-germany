...
const stripe = require("stripe")(process.env.STRIPE_SECRET_KEY);
 ...
 const geoCoder = mbxGeocoding({ accessToken: process.env.MAPBOX_API_KEY });
...
const newHotelPost = async (req, res) => {
    ...
    try {
        ...
        for (let file of req.files) {
         ...
         console.log(geoData.body.features[0].geometry.coordinates, "Çalıştı")
       hotel.geometry = geoData.body.features[0].geometry;

        await hotel.save();
         console.log(req.file, "çalıştı2")
        req.flash("success", "Otel oluştu");
        res.redirect(`/hotels/${hotel._id}`);
    } catch (error) {
        req.flash("error", "tekrar dene");
